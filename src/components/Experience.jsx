...
const Experience = () => {
    ...
    const certifi =[
     ...
     {
      id: 7,
      title:'Accelerate Application Development & Deployment with Flutter and AWS Amplify',
      date: 'May 05, 2022',
  ...
  return (
    <div name="experience" className="bg-[#09070A] text-[#EDECEC] w-full h-full ">
        ...
        <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 text-center py-8 px-12 sm:px-0'>
          ...
          {certifi.map(({id, title, date, sub, style,link}) => (
              <div key={id} className={`shadow-lg hover:scale-105 duration-500 py-2 rounded-lg ${style}`}>
                ...
                <button className='mt-2 p-2 rounded-md text-xs duration-200 hover:scale-105 hover:text-[#9B5642] bg-[#9B5642] hover:bg-[#EDECEC]'>View Certificate</button>  
                </a>           
            </div>
           ))}
        </div>
